...
{
    public class CreateMaintenancePackageRequestValidator : AbstractValidator<CreateMaintenancePackageRequestDto>
    {
        public CreateMaintenancePackageRequestValidator()
        {
            // ========== PACKAGE CODE ==========
            ...
            RuleFor(x => x.Status)
                ...
                .NotEqual(PackageStatusEnum.Deleted).WithMessage("Không thể tạo gói với trạng thái Deleted");
    ...
    /// <summary>
    /// Validator cho từng service item trong package
    /// </summary>
    public class PackageServiceItemRequestValidator : AbstractValidator<PackageServiceItemRequestDto>
    {
        public PackageServiceItemRequestValidator()
        {
            RuleFor(x => x.ServiceId)
                .GreaterThan(0).WithMessage("ServiceId phải > 0");

            RuleFor(x => x.QuantityInPackage)
                .GreaterThan(0).WithMessage("Số lượng dịch vụ trong gói phải > 0")
                .LessThanOrEqualTo(100).WithMessage("Số lượng dịch vụ trong gói không được vượt quá 100");

            When(x => x.AdditionalCostPerExtraQuantity.HasValue, () =>
            {
                RuleFor(x => x.AdditionalCostPerExtraQuantity!.Value)
                    .GreaterThanOrEqualTo(0).WithMessage("Chi phí thêm phải >= 0");
            });
        }
    }
}
